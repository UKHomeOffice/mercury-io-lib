...
class WebServiceSpec(implicit env: ExecutionEnv) extends Specification with BodyParsers with WebServiceSpecification {
  "Web service" should {
    """accept route starting with "/" and "get" subbed out""" in {
      withRoutes {
    ...
    """accept route not starting with "/" and "get" subbed out""" in {
      withRoutes {
        case GET(p"/my-get-route") => Action {
          Ok("Good Get")
        }
    ...
    """accept route starting with "/" and "post" subbed out""" in {
      withRoutes {
    ...
    """accept route not starting with "/" and "post" subbed out""" in {
      withRoutes {
        case POST(p"/my-post-route") => Action(parse.json) { request =>
          Ok(Json.obj("Good Post" -> request.body))
        }
